...
- **An external machine outside the house.** Essential. A machine on the home network
  reaches `44.xx.xx.xx/28` directly over VLAN 44 and never crosses the tunnel, so it cannot
  test the inbound path at all. `<external-host>` is used below.
...
ssh root@<hypervisor> "
  ...
  ip route add <internal-host>/32 via 44.xx.xx.xx dev eno1.44 src 44.xx.xx.xx
"
```

The extra route matters. Without it the host reaches internal addresses over its own LAN
connection, the traffic never passes the router, and the isolation rule is never exercised —
...
### 3. Allowed port from outside, delivered end to end
...
### 4. Allowed port to an address with nothing on it
...
Note the contrast with test 4: an allowed port to an empty address answers `No route to
...
### 6. Public segment into the internal network

```bash
ssh root@<hypervisor> "ping -c 3 -W 3 -I 44.xx.xx.xx <internal-host>"
```

Expect 100% loss, and the `cannot initiate into internal VLANs` counter up by exactly three.
Exact attribution matters — a result of 0/3 with no counter movement would mean the traffic
never reached the rule and the test proved nothing.

### 7. Router services from the public segment
...
`vmbr0v<tag>` per VLAN actually in use:

```bash
ssh root@<hypervisor> "ls /sys/class/net/ | grep -E 'eno1\.|vmbr0v' | sort | tr '\n' ' '"
ssh root@<hypervisor> "ip route get <internal-host>"
```
...
| 3 | Outside → `.82` udp 6526 | delivered, visible in capture |
| 4 | Outside → `.83` tcp 5525 | accepted, `No route to host` |
| 5 | Outside → `.82` tcp 80 / 22 | silent timeout |
| 6 | VLAN 44 → internal network | dropped, counter matches exactly |
| 7 | VLAN 44 → router services | silent timeout |
